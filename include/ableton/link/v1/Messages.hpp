...
struct MessageHeader
{
  ...
  friend It toNetworkByteStream(const MessageHeader& header, It out)
  {
    return discovery::toNetworkByteStream(
      header.messageType, std::move(out));
  }
  ...
  It out)
{
  ...
  if (messageSize < kMaxMessageSize)
  {
    return toNetworkByteStream(
      ...
      toNetworkByteStream(
        header,
        copy(begin(kProtocolHeader), end(kProtocolHeader), move(out))));
  }
  else
  {
...
std::pair<MessageHeader, It> parseMessageHeader(It bytesBegin, const It bytesEnd)
{
  ...
  MessageHeader header = {};
  const auto protocolHeaderSize =
    discovery::sizeInByteStream(detail::kProtocolHeader);
  ...
  if (std::distance(bytesBegin, bytesEnd) >= minMessageSize &&
      std::equal(begin(detail::kProtocolHeader), end(detail::kProtocolHeader), bytesBegin))
  {
