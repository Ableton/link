  ...
  for (path, dirs, files) in os.walk(path):
    for file in files:
      if file.endswith('cpp') or file.endswith('hpp'):
        ...
        clang_format_args = [ args.clang_format, '-style=file',
          '-output-replacements-xml', file_absolute_path]

        try:
          clang_format_output = subprocess.check_output(clang_format_args)
        except:
          logging.error('Could not execute {}, try running this script with the '
...
def check_formatting(args):
  ...
  if errors_found:
    logging.warn('Formatting errors found, please fix with clang-format -style=file -i')
  else:
